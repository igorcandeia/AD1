...
sum_data <- summariseData(data, 1, FALSE)

train <- sum_data[1:800, ]
test <- sum_data[801:925, ]
...
normalized_data_CRA <- as.data.frame(lapply(sum_data[6], normalize))
normalized_data <- sum_data

normalized_data[1] <- normalized_data_CRA
normalized_data <- normalized_data[,-1]
normalized_train <- normalized_data[1:800, ]
normalized_train_labels <- as.vector(normalized_train[["COD_EVASAO"]])
normalized_train <- normalized_train[,-1]
normalized_test <- normalized_data[801:925, ]
normalized_test_labels <- as.vector(normalized_test[["COD_EVASAO"]])
normalized_test <- normalized_test[,-1]

ctrl <- trainControl(method="repeatedcv",repeats = 3) 
knnFit <- train(COD_EVASAO ~ ., data = normalized_data[1:800, ], method = "knn", trControl = ctrl, preProcess = c("center","scale"), tuneLength = 20)
...
test <- read.csv("~/../Downloads/test_first_round_kaggle.csv", header = TRUE)
test_data <- summariseData(test, 1, TRUE)[,-1]

test_knn_data_normalized <- as.data.frame(lapply(test_data, normalize))
...
saida <- data.frame('COD_EVASAO' = evasao_pred,'MATRICULA' = as.vector(summariseData(test, 1, TRUE)[["MATRICULA"]]))
